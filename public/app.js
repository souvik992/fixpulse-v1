...
const resolveIssueUser = (bug, users, idKey, metadataLabel) => {
  const user = bug?.[idKey] ? users.find(u => u.id === bug[idKey]) : null;
  if (user) return user;
  const fallbackName = getMetadataValue(bug?.description, metadataLabel);
...
function BugDetail({ bugId, projects, users, onClose, onUpdate, onDelete, toast, currentUser }) {
  ...
  const assignee=bug?resolveIssueUser(bug,users,'assigneeId','Assignee(s)'):null;
    ...
    <div>
      ...
      <div className="table-card" style={{padding:20}}>
        ...
        {recentBugs.length===0?<div className="text-muted text-sm">No issues found.</div>:(
          ...
          <tbody>{recentBugs.map(bug=>{const assignee=resolveIssueUser(bug,users,'assigneeId','Assignee(s)');const reporter=resolveIssueUser(bug,users,'reporterId','Raised By');return(<tr key={`compact-${bug.id}`} onClick={()=>setSelectedBug(bug.id)}><td><span className="text-muted text-sm">{formatDate(bug.createdAt)}</span></td><td><span className="issue-title">{bug.title}</span></td><td>{reporter?<div style={{display:'flex',alignItems:'center',gap:6}}><Avatar user={reporter} size="xs"/><span style={{fontSize:12}}>{reporter.name}</span></div>:<span className="text-muted">—</span>}</td><td><TypeBadge t={bug.type}/></td><td>{assignee?<div style={{display:'flex',alignItems:'center',gap:6}}><Avatar user={assignee} size="xs"/><span style={{fontSize:12}}>{assignee.name}</span></div>:<span className="text-muted">—</span>}</td><td><PriorityBadge p={bug.priority}/></td><td><StatusBadge s={bug.status}/></td><td><span className="text-muted text-sm">{formatDate(bug.updatedAt)}</span></td></tr>);})}</tbody></table></div>
        )}
      </div>
    ...
    <div>
      ...
      {bugs.length===0?<div className="empty-state"><div className="icon">🎉</div><h3>No issues found</h3><p>Try adjusting your filters or create a new issue.</p></div>:(
        <div className="table-card">
          <div className="table-scroll"><table className="bug-table"><thead><tr><th>Date Created</th><th>Issue Title</th><th>Raised By</th><th>Issue Type</th><th>Assignee</th><th>Priority</th><th>Status</th><th>Last Updated</th></tr></thead>
          <tbody>{bugs.map(bug=>{const assignee=resolveIssueUser(bug,users,'assigneeId','Assignee(s)');const reporter=resolveIssueUser(bug,users,'reporterId','Raised By');return(<tr key={`compact-${bug.id}`} onClick={()=>setSelectedBug(bug.id)}><td><span className="text-muted text-sm">{formatDate(bug.createdAt)}</span></td><td><span className="issue-title">{bug.title}</span></td><td>{reporter?<div style={{display:'flex',alignItems:'center',gap:6}}><Avatar user={reporter} size="xs"/><span style={{fontSize:12}}>{reporter.name}</span></div>:<span className="text-muted text-sm">Unknown</span>}</td><td><TypeBadge t={bug.type}/></td><td>{assignee?<div style={{display:'flex',alignItems:'center',gap:6}}><Avatar user={assignee} size="xs"/><span style={{fontSize:12}}>{assignee.name}</span></div>:<span className="text-muted text-sm">Unassigned</span>}</td><td><PriorityBadge p={bug.priority}/></td><td><StatusBadge s={bug.status}/></td><td><span className="text-muted text-sm">{formatDate(bug.updatedAt)}</span></td></tr>);})}</tbody></table></div>
